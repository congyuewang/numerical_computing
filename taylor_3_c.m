function [t,x] = taylor_3_c(tRange, x0, n)
...
for k = 1 : length(t)-1
   x1 =  1/t(k)^2-x(k)/t(k)-x(k)^2;
   x2 = -2*t(k)^(-3)-x1/t(k)+x(k)*t(k)^(-2)-2*x(k)*x1;
   x3 = 6*t(k)^(-4)-x2/t(k)+2*x1*t(k)^(-2)-2*x(k)*t(k)^(-3)-2*x1*x1-2*x(k)*x2;
   x4 = -24*t(k)^(-5)-x3/t(k)+3*x2/t(k)^2-6*x1/t(k)^3+6*x(k)/t(k)^4-6*x1*x2-2*x3*x(k);
