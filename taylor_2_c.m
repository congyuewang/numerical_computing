function [t,x] = taylor_2_c(tRange, x0, n)
...
x(1) = x0;
for k = 1 : length(t)-1
   x1 =  t(k)*x(k)+t(k)^2*x(k)^2;
   x2 = x(k)+t(k)*x1+2*t(k)*x(k)^2+2*t(k)^2*x(k)*x1;
   x3 = 2*x1 + t(k)*x2+2*x(k)^2+8*t(k)*x(k)*x1+2*t(k)^2*x1*x1+2*t(k)^2*x(k)*x2;
